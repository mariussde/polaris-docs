...
Setting up PolarisDocs is straightforward, with options to clone the repository or use the convenient `npx` command. PolarisDocs requires [Node.js](https://nodejs.org/) version 18 or higher for optimal performance.

## Clone Repository
To get started, you can clone the PolarisDocs repository directly from GitHub. 
<Stepper>
  <StepperItem title="Step 1: Clone the PolarisDocs Repository">
    Begin by cloning the PolarisDocs repository from GitHub:
...
For a faster setup, use the `npx` command to create a new PolarisDocs project in one step:
...
With this setup, you’ll have a ready-to-use PolarisDocs instance to start building your documentation.